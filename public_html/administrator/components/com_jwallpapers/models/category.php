 ...
 * @package JWallpapers
 * @copyright Copyright (C) 2009 Arunas Mazeika, http://www.scriptz-team.info. All rights reserved
	...
	function getCategoryChilds($id = null) {
		...
		if ($id === null) { 
			...
			if (! $this->_catChildsList) {
				$query = 'SELECT title, id FROM #__jwallpapers_categories WHERE parent_id = ' . 
										( int ) $this->_id . 
										' order by title';
